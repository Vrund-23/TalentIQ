...
async function captureProblem(problemUrl) {
    ...
    try {
        ...
        if (problemUrl.includes('leetcode.com')) {
            ...
            // Try to wait for generic problem container if possible, but don't crash
            try {
...
exports.captureScreenshot = async (req, res) => {
    try {
        ...
        try {
            ...
            return res.status(500).json({ error: "Failed to capture screenshot: " + botError.message });
        }
        ...
        return res.status(500).json({ error: "Failed to process problem", details: error.message });
    }
};
